...
// keys, to be resolved down to an Tezos address - which will be kept in a LRU cache.
// This supports cases where the signing device behind Tezos is able to support keys
// addressed using somthing like a HD Wallet hierarchical syntax.
// Once the resolver has returned the String->Address mapping, the tezosconnect downstream
// signing process must be able to process using the resolved tezos address (meaning
...
func (ar *addressResolver) ResolveSigningKey(ctx context.Context, keyDescriptor string, intent blockchain.ResolveKeyIntent) (string, error) {
	...
	address, err := formatTezosAddress(ctx, jsonRes.GetString(ar.responseField))
	if err != nil {
